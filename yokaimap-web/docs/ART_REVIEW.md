...
- **비용은 장당 8크레딧**이다(`recraft_v4_1` · 2k · standard, 2026-09 실측: 12장에 96).
  100체를 새로 그리면 800크레딧이므로, 도감을 늘릴 때 데이터보다 도상이 먼저 막힌다.
  확장 계획을 세울 때 이 숫자부터 본다.
- **앵커가 장면과 맞는지 프롬프트 단계에서 본다.** `category_anchors`는 분류 단위라
  뭉툭하다. `objects` 앵커는 달항아리·소반·장독대를 넣는데, 탈판·고분 천장·부적 판화·
  무기고에는 그 소품이 놓일 자리가 없다. `landscape`도 소나무·서낭당을 넣으므로
