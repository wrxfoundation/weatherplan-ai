...
- 데이터 v0.1.0 — **요괴·신격 163체 / 설화 29편 / 노래·시가 34편 / 전승지·배경지 126곳(중복 좌표 제외 112곳) · 62개 시군구 / 17개 시도 전체 커버**
...
`VITE_SITE_ORIGIN`은 빌드 시점에 프리렌더 266페이지의 canonical에 구워지므로
...
## 도상(圖版) 파이프라인 — minhwa-v2

한국에는 도리야마 세키엔 같은 퍼블릭도메인 요괴 도감이 없어 도상을 전량 새로 만든다.
그래서 이 파이프라인의 목표는 두 가지다. **① 600장을 찍어도 화풍이 흔들리지 않을 것, ② 왜색·중국풍이 섞이지 않을 것.**
두 번째는 품질 문제가 아니라 프로젝트 존폐가 걸린 문제라, 배제 항목을 데이터로 관리하고 사람이 검수한다.

```
data/art/direction.json     아트디렉션 minhwa-v2
                              · style        한국 안료·기법 (한지 + 아교 갠 석채 + 철선묘 + 무광)
                              · palette      오방색 계열 전통 안료색(주사·석록·석청·자황·호분·먹)
                              · korean_anchors  한복(동정·옷고름)·한옥·경물·기물 — 매 컷 강제
                              · exclude_japanese / exclude_chinese  배제 마커 + 검수 체크리스트
...
**현황: 도판 132장 반입·검수 완료 · 31체 미생성(인장 폴백). 설화는 도판을 쓰지 않는다.**
...
- **한국성 앵커를 매 컷에 강제한다.** 저고리 동정·옷고름, 갓·망건, 한옥 격자문, 서낭당 오색천·장승·솟대, 놋그릇·달항아리.
  이걸 넣지 않으면 모델은 "동아시아 평균값"으로 떨어진다.
- **배제 마커를 프롬프트에 명시한다.** 기모노·오비·도리이·시메나와·우키요에 판화풍·다다미·벚꽃 /
  치파오·한푸·변발·오조룡·홍등롱·년화식 금박. 배제문은 스타일 바로 뒤에 두어, 프롬프트가 잘리더라도 먼저 살아남게 했다.
- **도깨비에 뿔을 그리지 않는다.** 뿔 달린 도깨비는 일본 오니 도상이 근대 학습물을 통해 들어온 결과라는 지적이 있고,
  그 서술을 우리 데이터(`kr-dokkaebi`)에 이미 실어 두었다. 도상에서 되풀이하면 자기모순이다.
- `art_hint`가 없는 개체는 **생성 대상에서 제외**된다(한국어 traits는 이미지 모델이 해석하지 못한다).
...
검수 체크리스트: **[docs/ART_REVIEW.md](./docs/ART_REVIEW.md)**
...
│   └── yokai/01~14-*.json         # 시드 163체 (카테고리별 분할)
├── scripts/
