// 콘솔 전용 목데이터 — 경영(admin) · 관제(dispatch) · 컨시어지(concierge) 화면에서만 쓴다.
//
// mock.js 에서 분리한 이유: 이 데이터는 180 KB 가 넘는데, 한 파일에 같이 두면
// webpack 이 공용 청크로 묶어 어르신·가족 앱까지 내려보낸다.
...
// 어르신 댁 좌표 — 관제 프로필의 '위치 지도'와 보호자 홈의 '지금 어디쯤'에 쓴다
// (2026-08-31 요청). 명부의 동(洞)에 맞춘 좌표이고 번지까지는 두지 않는다 —
// "상세 주소는 담당 확정 후에만 노출"이라는 명부 원칙과 같은 선이다.
// 김순자는 mapPeople 의 좌표와 같은 값을 쓴다 (관제 맵과 팝업이 다른 자리를
// 가리키면 안 된다).
export const ELDER_HOMES = {
  김순자: { lat: 37.4945, lng: 127.0614, dong: "대치동" },
  이영호: { lat: 37.5132, lng: 127.1029, dong: "잠실동" },
  박말순: { lat: 37.5379, lng: 127.1425, dong: "길동" },
  한복자: { lat: 37.5361, lng: 127.1398, dong: "길동" },
  오태식: { lat: 37.5006, lng: 127.0364, dong: "역삼동" },
  최정자: { lat: 37.4813, lng: 126.9971, dong: "방배동" },
};

// 컨시어지 현재 위치 — mapPeople 과 같은 좌표. 이동 중인 사람만 담는다.
export const CONCIERGE_POS = {
  박지현: { lat: 37.5029, lng: 127.0567, state: "이동중" },
  정민호: { lat: 37.4956, lng: 126.8974, state: "수행중" },
  한서연: { lat: 37.5219, lng: 126.9895, state: "대기" },
};
...
export const VIDEO_POLICY = {
  ...
  // 곳이라 안전진단·안전바 설치 확인에 사진이 필요하다. 다만 사람이 있을 때는
...
// 오늘의 짝 — 2인 지정 방문의 실행 UI. "짝 없이 들어가지 마세요"가 원칙의 실행 카피
...
export const PROTECT_RULES = [
  // 2026-08-13 개편안 V2 — 전면 2인에서 위험도 기반으로 바뀌었다 (lib/dispatch-policy.js)
  ["2인 지정 방문 단독 진입 금지", "첫 방문 · 할아버지 1인가구 · 종합평가 · 동행 프리미엄은 짝이 없으면 배차하지 않음 · 고객 요청으로도 축소 불가"],
  ["1인 방문 중 위험 감지 시", "즉시 관제 보고 · 현장 이탈 가능 · 2인 재방문으로 전환 · 이탈로 인한 불이익 없음"],
...
export const PAIR_WHY = [
  ["동행자 보호", "성희롱 · 폭언의 목격자", "위험이 큰 자리에서는 단둘이 있는 상황 자체를 만들지 않습니다. 발생 시 두 사람이 함께 이탈하고 진술이 두 개 남습니다."],
  ["고객 보호", "도난 · 금품 의혹 차단", "어르신 가정에서 물건이 없어졌다는 의혹은 증명도 반증도 어렵습니다. 첫 방문에서 2인 동시 입실이 양쪽을 지킵니다."],
...
export const PAIR_RULES = [
  "2인 지정: 첫 안심방문 · 할아버지 1인가구 · 종합평가 · 병원동행 프리미엄 (lib/dispatch-policy.js)",
  "1인 가능: 요양병원 안심방문 · 할머니 1인 거주 정기방문 · 부부 거주 · 동행 베이직 · 물품 전달",
  "동성 페어를 우선 배정하되, 어르신 성별과 동일한 성별이 최소 1명 포함되도록 강제",
  "같은 페어가 4회 연속 배정되지 않도록 순환 — 담합·유착 방지",
  "수습은 반드시 부 동행으로만 시작 · 주 동행 승격은 부 동행 12건 이후",
  "2인 지정 방문은 짝을 찾지 못하면 예약 자체를 받지 않음 — 단독 배차로 대체하지 않음",
  "분류되지 않은 새 유형은 2인으로 배차 — 판단이 안 서면 안전한 쪽",
];

export const PAIR_STATS = [
  { k: "2인 지정 준수율", v: "100%", note: "2인 대상 방문의 단독 배차 0건 · 예외 승인 절차 없음" },
  { k: "성희롱 · 폭언 신고", v: "2건", note: "모두 2인 진술 확보 · 고객 계약 해지" },
  { k: "1인 방문 비중", v: "미집계", note: "개편안 V2 배차 규칙 적용 후 산출 (2026-08-13~)" },
...
export const RISK_REGISTER = [
  ...
  { id: "H11", name: "가정 내 성희롱 · 폭언 · 금품 의혹 (양방향)", detail: "발생 중간 × 영향 큼 · 브랜드 존립·형사", act: "위험 유형은 2인 배차 강제 · 짝 미매칭 시 예약 거부 · 2인 진술 기록 · 1인 방문 중 감지 시 즉시 이탈 · 고객 계약 해지 권한", owner: "운영 · 법무", state: "완화됨", prob: 2, impact: 2.7 },
...
export const CRM_STUCK = {
  ...
  pay: [
    ...
    { code: "P-029", days: "12일 정체", signal: "견적 후 무응답 · 경쟁 서비스 비교 추정", act: "차별점 자료(감사 로그 · 배차 규칙) 발송 + 지점장 콜", owner: "지점장" },
...
export const WEAR_RULES = [
  { k: "낙상 (기기 감지)", cond: "Fit3 내장 감지 + 60초 미취소 → 폰 긴급 SOS 실행", fired: "4건", real: "3건", falseRate: "25%", route: "관제 SMS 게이트웨이 → 자동 티켓 + 보호자 동시", tone: "ok" },
...
export const WEAR_PIPELINE = [
  ["기기 (갤럭시 Fit3)", "제약 확인", "밴드는 서드파티 앱 설치 불가 — 손목에 우리 화면 · 버튼을 못 올린다", "warn"],
  ["Samsung Health", "정상", "기기 데이터 1차 수집 · 안드로이드 폰 필수 (iOS 미지원)", "ok"],
  ["Health Connect", "정상", "심박 · 걸음 · 수면 · SpO₂ 표준 타입만 넘어온다", "ok"],
  ["컴패니언 앱 (Android)", "개발 예정", "Health Connect는 네이티브 API — 웹에서 못 읽는다", "warn"],
  ["낙상 이벤트", "SMS 경로", "수신 API 없음 — 폰 긴급 SOS 문자를 관제 게이트웨이로 받는다", "warn"],
  ["착용 여부", "추정", "전용 신호 없음 — 심박 레코드 연속성으로 판정", "warn"],
  ["기기 배터리 · 펌웨어", "수기", "서드파티에 미공개 — 방문 점검 시 기록", "bad"],
  ["스트레스 지표", "미연동", "삼성 독자 지표 — 파트너 SDK 필요", "bad"],
  ["위치 (GPS)", "폰 GPS", "밴드 단독 GPS 미탑재 — 긴급 문자의 위치도 폰 기준", "info"],
];

// ════ 119 신고 프로토콜 — 2026-08-01 회의 확정 ════
// 원칙: 신고는 하되 흔적을 남긴다. 연락이 닿지 않아 부득이 신고한 경우에도
//       "몇 번 걸었고 무엇을 남겼는가"가 기록으로 있어야 사후에 다툼이 없다.
export const SOS_119 = [
  { s: "01", k: "어르신 본인 3콜", t: "0~90초", body: "관제가 직접 3회 발신. 응답하면 상태 확인 후 종결 또는 컨시어지 급파", tone: "info" },
  { s: "02", k: "보호자 2콜", t: "+60초", body: "본인 미응답 시 주 보호자에게 2회. 부 보호자는 동시 문자 발송", tone: "info" },
  { s: "03", k: "조치 통보 남기기", t: "+30초", body: "둘 다 안 되면 “이렇게 조치하겠습니다”를 문자·앱 알림으로 남긴다 — 남긴 사실이 곧 근거", tone: "warn" },
  { s: "04", k: "119 신고", t: "즉시", body: "관제가 신고. 신고 직후 119에서 신고자에게 회신 전화가 오므로 관제가 상황을 설명한다", tone: "bad" },
  { s: "05", k: "현장 합류", t: "20분 내", body: "근접 컨시어지 급파. 문 개방이 필요하면 사전 동의 범위에서 진행", tone: "bad" },
  { s: "06", k: "보호자 역할 대행", t: "도착 시", body: "원거리 자녀가 도착하기까지(평균 4시간) 병원에서 보호자 역할을 임시 대행한다", tone: "ok" },
];

export const SOS_CONSENT = {
  head: "가입 시 사전 동의로 받아 두는 것 — 없으면 그 순간 아무것도 못 한다",
  items: [
    "긴급상황에서 본인 · 보호자와 연락이 닿지 않을 때 부득이 119에 신고하는 것",
    "구호 조치를 위해 출입문 개방이 필요할 수 있다는 것",
    "신고 · 조치 내용을 보호자에게 사후 통보하는 것",
    "병원에서 보호자 도착 전까지 우리가 보호자 역할을 임시 대행하는 것",
  ],
  note: "동의는 가입 화면에서 별도 항목으로 받습니다 — 약관에 묻어 두지 않습니다.",
};

// ════ 낙상 대응 — 감지는 기기가, 대응은 우리가 ════
// Fit3에는 우리 앱을 못 올린다. 낙상 '감지'를 우리가 만들 방법은 없다.
// 그래서 기기의 긴급 SOS가 우리에게 닿도록 경로만 설계하고, 그 다음부터를 우리가 책임진다.
export const FALL_CHAIN = [
  { s: "01", who: "기기", k: "충격 감지", t: "0초", body: "Fit3 내장 알고리즘 — 임계값 조정도 원시값 수신도 불가한 구간", tone: "info" },
  { s: "02", who: "기기", k: "60초 카운트다운", t: "0~60초", body: "어르신이 취소하면 종료. 오탐의 대부분이 여기서 걸러진다", tone: "info" },
  { s: "03", who: "폰", k: "긴급 SOS 실행", t: "60초", body: "미취소 시 폰이 등록된 긴급 연락처로 전화 + 위치 문자 발송", tone: "warn" },
  { s: "04", who: "K-CARE", k: "SMS 게이트웨이 수신", t: "+5초", body: "관제 수신 번호를 긴급 연락처 1순위로 등록 — 문자가 웹훅으로 들어온다", tone: "ok" },
  { s: "05", who: "K-CARE", k: "SOS 티켓 자동 생성", t: "+10초", body: "발신 번호로 가구 매칭 · 위치 · 복약 · 병력 · 담당 · 보호자 순서 자동 첨부", tone: "ok" },
  { s: "06", who: "K-CARE", k: "3단 대응", t: "+30초~", body: "콜백 → 근접 컨시어지 급파 → 119 연계. 여기부터가 우리 서비스다", tone: "ok" },
];

export const FALL_SCOPE = {
  can: [
    "낙상 발생 사실 · 시각 · 위치를 30초 안에 관제 콘솔에 띄운다",
    "가구 프로필(복약 · 병력 · 주 담당 · 보호자 순서)을 티켓에 자동 첨부한다",
    "취소된 낙상(오탐)도 기록으로 남겨 대응 룰 개선에 쓴다",
    "긴급 연락처 등록 상태를 가구별로 관리한다 — 미등록이 곧 사각지대",
  ],
  cant: [
    "낙상 감지 알고리즘 자체 — 기기 내장이라 민감도 조정 불가",
    "가속도 원시값 수신 — Health Connect가 제공하지 않는다",
    "밴드 화면에 K-CARE UI · 취소 버튼 커스터마이즈",
    "폰이 꺼졌거나 밴드와 떨어져 있을 때의 감지 · 전달",
  ],
};

// 긴급 SOS 문자에서 뽑아 쓸 수 있는 것 — 실물 실측 후 확정할 파싱 규격
export const FALL_SMS = [
  { f: "발신 번호", v: "010-****-1234", use: "가구 매칭 키 — 사전 등록이 안 되면 티켓이 안 만들어진다", tone: "ok" },
  { f: "수신 시각", v: "14:22:07", use: "SLA 기산점", tone: "ok" },
  { f: "위치", v: "지도 링크 (위경도)", use: "근접 컨시어지 산출 · 119 위치 전달", tone: "ok" },
  { f: "본문 문구", v: "제조사 정형 문구", use: "낙상 / 수동 SOS 구분 — 실측으로 확정 필요", tone: "warn" },
  { f: "심박 · 배터리", v: "미포함", use: "문자에 없다 — 티켓에서 Health Connect 값으로 보충", tone: "bad" },
];

// 가구별 준비 상태 — 이 표의 붉은 줄이 곧 '낙상이 나도 우리에게 안 오는 가구'
export const FALL_READY = [
  { who: "김순자 (78)", phone: "등록", sos1: "관제 게이트웨이", sos2: "김민수 (주 보호자)", fall: "켜짐", state: "정상", tone: "ok" },
  { who: "이영호 (81)", phone: "등록", sos1: "관제 게이트웨이", sos2: "이도현", fall: "켜짐", state: "정상", tone: "ok" },
  { who: "도금례 (86)", phone: "등록", sos1: "관제 게이트웨이", sos2: "도예진", fall: "켜짐", state: "정상", tone: "ok" },
  { who: "박말순 (83)", phone: "등록", sos1: "박정훈 (보호자)", sos2: "—", fall: "켜짐", state: "관제 미등록 — 낙상이 나도 우리에게 안 온다", tone: "bad" },
  { who: "한복자 (79)", phone: "등록", sos1: "관제 게이트웨이", sos2: "한지우", fall: "꺼짐", state: "낙상 감지 OFF — 설정 방문 필요", tone: "bad" },
  { who: "안병철 (85)", phone: "미소지", sos1: "—", sos2: "—", fall: "—", state: "폰 미소지 — Fit3 경로 불가 · 응급버튼 대상", tone: "bad" },
];

// 2주 실증 — 문서로는 안 나오는 것부터 실물로 확인한다
export const FALL_PLAN = [
  { d: "D1~2", k: "실물 확보 · 문자 원문 수집", who: "개발", body: "Fit3 2대 + 안드로이드 폰 2대. 긴급 SOS 문자를 원문 그대로 모은다 (문구 · 필드 · 위치 형식)" },
  { d: "D3~5", k: "SMS 수신 게이트웨이", who: "개발", body: "회사 수신 번호 개통 → 웹훅. 문자 → 티켓 파서 (발신번호 매칭 · 위치 추출)" },
  { d: "D6~8", k: "낙상 재현 시험", who: "운영 · 개발", body: "매트 위 낙상 20회 + 일상 동작 100회(앉기 · 눕기 · 계단) — 감지율과 오탐률 실측" },
  { d: "D9~10", k: "관제 티켓 연결", who: "개발", body: "SOS 대응 센터에 자동 티켓 · 3단 대응 타이머 연결" },
  { d: "D11~12", k: "가구 설정 절차 확정", who: "운영", body: "컨시어지 방문 체크리스트 — 긴급 연락처 순서 · 낙상 감지 ON · 취소 방법 교육" },
  { d: "D13~14", k: "파일럿 5가구", who: "운영", body: "동의받은 5가구 적용. 오탐 1건당 대응 비용까지 기록한다" },
];

export const FALL_METRICS = [
  { k: "감지율", target: "≥ 90%", how: "재현 낙상 20회 중 알림 발생", note: "기기 성능 — 못 미치면 우리가 고칠 수 없다. 응급버튼 병행 판단 기준" },
  { k: "오탐률", target: "≤ 15%", how: "일상 동작 100회 중 오알림", note: "60초 취소가 대부분 거른다. 취소된 건도 기록은 남긴다" },
  { k: "우리 도달", target: "≤ 30초", how: "낙상 → 관제 티켓 생성", note: "여기부터가 우리 책임 구간 — 계약서에 쓸 수 있는 숫자" },
  { k: "첫 접촉", target: "≤ 60초", how: "티켓 → 관제 콜백 발신", note: "SOS 대응 센터 SLA와 동일하게 맞춘다" },
  { k: "현장 도착", target: "≤ 20분", how: "티켓 → 컨시어지 도착", note: "권역 밀도에 종속 — 지점 지표와 직결" },
];
...
export const SOS_TEAM = [
  ...
  { role: "부 컨시어지", name: "서다인", detail: "지정 짝 · 차량 보유 · 12분 거리", state: "급파 대기", tone: "warn", action: "동반 배정" },
...
export const COMPETITORS = [
  { k: "건별 매칭 플랫폼", price: "건당 과금", pair: "단독 동행", moat: "가격 경쟁 · 동행자 이탈률 높음", us: "구독 + 위험도 기반 배차로 안전·지속 소득", tone: "warn" },
...
// 결론부터: 글래스로 가면 비용의 90%가 클라우드가 아니라 하드웨어와 그 운영이다.
// 전제 — 가구당 월 4회 동행 · 회당 2시간 · 컨시어지 1인당 담당 5.3가구
//        기기는 1인 1대가 아니라 지점 공용(동행 시 불출)으로 계산한다
export const REC_DEVICES = [
  { id: "D", k: "바디캠 (가슴 클립)", ex: "보안 · 현장 표준 제품군", price: "35만", state: "권장",
    pros: ["배터리 8~12시간 — 하루를 통째로 커버한다", "사전 녹화 버퍼 30초 — 버튼을 늦게 눌러도 사건 직전이 남는다", "IP67 · 낙하 내구 · 야간 촬영 · GPS 내장", "도크 자동 업로드가 표준 — 개발이 거의 없다"],
    cons: ["가슴 높이라 시선이 어긋난다 (의사가 가리킨 화면을 놓칠 수 있다)", "실시간 원격 지원은 LTE 모델만 — 단가 2~3배 + 회선비", "부축 중 버튼 조작이 어렵다 (사전 녹화 버퍼가 상당 부분 보완)"], tone: "ok" },
  { id: "B", k: "중급 카메라 글래스", ex: "일상 안경형 · 앱 연동", price: "80만", state: "병행",
    pros: ["1인칭 시점 — 시선이 일치한다", "음성 제어 핸즈프리", "일상 안경 외형"],
    cons: ["배터리 2시간 — 동행 1회가 한계, 예비 운영 필수", "사전 녹화 버퍼 없음", "내구성 · 방수 취약"], tone: "warn" },
  { id: "A", k: "엔터프라이즈 글래스", ex: "RealWear · Vuzix 급", price: "250만", state: "특수 용도",
    pros: ["안드로이드 — 우리 앱을 기기에 올린다", "관제 실시간 원격 지원 · MDM · 핫스왑 배터리"],
    cons: ["산업용 외형 — 케어 현장에 과하다", "10,000가구 전면 도입 시 13.7억"], tone: "info" },
  { id: "C", k: "컨슈머 글래스", ex: "Ray-Ban Meta 류", price: "35만", state: "불가",
    pros: ["가장 트렌디 · 외형 거부감 최소"],
    cons: ["영상이 제조사 클라우드를 경유 — 우리가 통제 못 한다", "파이프라인 API 없음 · 민감정보 처리위탁 요건 미충족"], tone: "bad" },
];

// 바디캠 vs 글래스 — 무엇이 실제로 다른가
export const REC_COMPARE = [
  { k: "배터리", cam: "8~12시간 (하루 종일)", gl: "2시간 (동행 1회)", win: "cam" },
  { k: "사전 녹화 버퍼", cam: "30초 — 낙상 직전이 남는다", gl: "대부분 없음", win: "cam" },
  { k: "내구성", cam: "IP67 · 낙하 · 야간", gl: "생활 방수 수준", win: "cam" },
  { k: "대당 단가", cam: "35만", gl: "80만", win: "cam" },
  { k: "업로드", cam: "도크 자동 — 개발 최소", gl: "앱 페어링 구축 필요", win: "cam" },
  { k: "시선 일치", cam: "가슴 높이 — 어긋남", gl: "1인칭 — 정확", win: "gl" },
  { k: "핸즈프리", cam: "버튼 조작", gl: "음성 명령", win: "gl" },
  { k: "실시간 원격 지원", cam: "LTE 모델만 (고가)", gl: "가능", win: "gl" },
];

// 규모별 총비용 — 두 안 나란히 (구간 녹화 8분 · 720p H.265 · 30일 보관 공통)
// 대수 산정: 글래스는 배터리 2h라 동행 회 단위 불출, 바디캠은 8h+라 하루 단위 불출
export const REC_SCALE = [
  { n: "1,000가구", staff: "189명",
    cam: { units: "84대", capex: "2,940만", opex: "249만", per: "2,490원",
      detail: [["기기 감가 (3년)", "82만"], ["분실 · 파손 (연 8%)", "20만"], ["도크 · 소모품", "9만"], ["클라우드 · 서버 · 보안", "58만"], ["기기 운영 인력", "80만"]] },
    gl: { units: "55대", capex: "4,400만", opex: "313만", per: "3,130원" } },
  { n: "5,000가구", staff: "943명",
    cam: { units: "419대", capex: "1.47억", opex: "793만", per: "1,586원",
      detail: [["기기 감가 (3년)", "408만"], ["분실 · 파손 (연 8%)", "98만"], ["도크 · 소모품", "44만"], ["클라우드 · 서버 · 보안", "93만"], ["기기 운영 인력", "150만"]] },
    gl: { units: "273대", capex: "2.18억", opex: "1,062만", per: "2,124원" } },
  { n: "10,000가구", staff: "1,887명",
    cam: { units: "840대", capex: "2.94억", opex: "1,446만", per: "1,446원",
      detail: [["기기 감가 (3년)", "817만"], ["분실 · 파손 (연 8%)", "196만"], ["도크 · 소모품", "88만"], ["클라우드 · 서버 · 보안", "145만"], ["기기 운영 인력", "200만"]] },
    gl: { units: "546대", capex: "4.37억", opex: "1,986만", per: "1,986원" } },
];

// 단계 전략 — 바디캠으로 깔고, 원격 지원이 필요한 곳에만 글래스를 얹는다
export const REC_HYBRID = [
  { d: "1단계", k: "바디캠 전면", body: "기록 · 분쟁 방어 · 낙상 직전 캡처는 바디캠이 더 잘한다. 전 지점 기본 장비로", cost: "10,000가구 월 1,446만", tone: "ok" },
  { d: "2단계", k: "글래스 소수 병행", body: "수습 교육 · 고난도 동행에만 LTE 글래스 10~20대 — 관제 원격 지원 전용", cost: "20대 기준 월 60만 추가", tone: "warn" },
  { d: "3단계", k: "효과로 판단", body: "원격 지원이 수습 전환 기간을 줄였는지 측정. 효과 없으면 글래스는 접고 바디캠만 남긴다", cost: "—", tone: "info" },
];

export const GLASS_MODES = [
  { k: "전 구간 1080p", size: "회당 4.5GB", cloud10k: "월 1,090만", state: "불가", why: "보관 1년이면 월 7,500만 — 사업이 안 된다", tone: "bad" },
  { k: "전 구간 720p H.265", size: "회당 1.36GB", cloud10k: "월 450만", state: "비권장", why: "컨시어지 1인당 월 29GB 업로드 — 통신비까지 밀린다", tone: "warn" },
  { k: "구간 녹화 8분 (권장)", size: "회당 0.09GB", cloud10k: "월 145만", state: "권장", why: "진료실 대화 · 투약 설명 · 이상 상황만. 전 구간 대비 50배 절감", tone: "ok" },
];

// 보관 · 이관 정책 — 우리는 의료기관이 아니라 진료기록 10년 보존 대상이 아니다
export const GLASS_RETENTION = [
  { k: "일반 동행", term: "30일 자동 파기", store: "S3 Standard (서울 리전)", why: "목적 달성 시 지체 없이 파기 — 길게 둘 이유가 없다", tone: "ok" },
  { k: "이슈 태깅", term: "1년", store: "Glacier IR", why: "민원 · 낙상 · 투약 오류 — 전체의 약 5%", tone: "warn" },
  { k: "분쟁 · 수사", term: "리걸 홀드", store: "Object Lock", why: "파기 정지 · 사건 종결 후 파기. 무기한 보관이 아니다", tone: "bad" },
];

export const GLASS_TRANSFER = [
  { q: "국내에 둬야 하나?", a: "의무는 없다. 다만 해외 리전은 국외이전이라 별도 고지 · 동의가 필요하다(§28의8). 서울 리전을 쓰면 이 문제가 통째로 사라진다.", tone: "ok" },
  { q: "다른 클라우드로 옮겨도 되나?", a: "된다. 처리위탁이라 동의는 불필요하고 공개 의무만 있다 — 처리방침에 수탁자 공개 · 위탁계약 체결 · 이전 중 암호화 유지 · 이전 자체를 감사 로그로 기록.", tone: "ok" },
  { q: "사내 서버(NAS)에 보관해도 되나?", a: "권장하지 않는다. 암호화 · 접근기록 · 백업 · 물리보안 수준을 클라우드만큼 맞추기 어렵고 사고 책임이 전부 회사에 온다. 굳이 한다면 암호화된 오프라인 콜드 백업만.", tone: "bad" },
  { q: "제3자 제공과 뭐가 다른가?", a: "위탁은 우리 목적을 대신 처리하는 것(동의 불필요), 제공은 상대가 자기 목적으로 쓰는 것(동의 필요). 클라우드 이전은 위탁이다.", tone: "info" },
];

// 현장 운영 규칙 — 트렌드를 살리되 거부감은 설계로 관리한다
export const GLASS_FIELD = [
  { k: "촬영 표시 상시", body: "녹화 중 LED 점등 + 시작 시 음성 고지. 이동형 영상기기는 촬영 사실을 알 수 있게 표시해야 한다 (§25의2)", tone: "ok" },
  { k: "한마디로 중단", body: "어르신이 “지금 끄세요” 하면 버튼 한 번(바디캠) 또는 음성(글래스)으로 즉시 중단 · 그 사실도 기록에 남긴다", tone: "ok" },
  { k: "병원 자동 중단", body: "지오펜스로 병원 진입 시 자동 OFF. 진료실은 병원 MOU에 촬영 조항이 있고 사전 동의된 경우에만 재개", tone: "warn" },
  { k: "첫 3회는 미착용", body: "라포가 생긴 뒤 도입한다. 첫 방문부터 카메라를 쓰면 해지 사유가 된다", tone: "ok" },
  { k: "거부해도 동일 서비스", body: "촬영 거부 가구도 서비스 차이 없음을 계약서에 명시. 거부율은 지표로 관리한다", tone: "ok" },
  { k: "제3자 최소화", body: "대기실 · 복도는 녹화 대상이 아니다. 구간 녹화가 곧 제3자 노출 최소화 장치", tone: "warn" },
];

// 글래스를 선택하는 진짜 이유 — 기록만이면 바디캠이 싸다
export const GLASS_ONLY = [
  { k: "관제 실시간 원격 지원", body: "수습 컨시어지의 시야를 시니어 코치가 원격으로 보며 지시한다. 마포지점 수습 비중 50% 문제를 사람 파견 없이 푼다", ir: "채용 → 투입 리드타임 31일 단축의 지렛대", tone: "ok" },
  { k: "1인칭 시점", body: "의사가 무엇을 가리켰는지 · 어르신이 무엇을 보고 불안해했는지가 남는다. 가슴 바디캠은 시선이 어긋난다", ir: "리포트 신뢰도 · 분쟁 방어", tone: "ok" },
  { k: "완전 핸즈프리", body: "부축 · 휠체어 조작 중에도 음성으로 시작 · 중단. 바디캠은 사전 녹화 버퍼 30초가 이걸 상당 부분 대신한다", ir: "손이 묶이는 순간이 기록이 필요한 순간", tone: "warn" },
];

export const REC_PHASES = [
  { d: "1단계 · 3주", k: "기기 선정 · 실물 검증", body: "바디캠 후보 3종 대여 — 연속 촬영 시간 · 사전 녹화 버퍼 동작 · 무게 · 도크 업로드 속도 · 어르신 반응", who: "개발 · 운영" },
  { d: "2단계 · 6주", k: "병원 MOU 촬영 조항", body: "제휴 6곳 재협상에 촬영 조항 추가. 이게 없으면 기기를 사도 진료실에서 못 쓴다 — 실제 게이트", who: "제휴" },
  { d: "3단계 · 4주", k: "동의 · 파기 체계", body: "민감정보 별도 동의 · 30일 자동 파기 · 열람 승인제 · 감사 로그. PIA에 항목 포함", who: "법무 · 개발" },
  { d: "4단계 · 8주", k: "1개 지점 파일럿", body: "강남 본점 30대. 거부율 · 중단 요청 빈도 · 실제 활용률 · 분실 파손률 측정", who: "운영" },
  { d: "5단계", k: "확대 · 글래스 병행 판단", body: "거부율 20% 초과 또는 활용률 40% 미만이면 중단. 원격 지원 수요가 확인되면 그때 글래스 10~20대 추가", who: "경영" },
];

// ════ 방향성 회의 확정 (2026-08-01) — 매출 정의 · 수익 배분 · 의사결정 ════
export const REV_DEF = {
  include: [
    { k: "중개 수수료", body: "외주 · 제휴 서비스 연결 (대리운전 · 상조 · 매칭 등)" },
    { k: "광고 수수료", body: "병원 입점 · 노출 · 건강기능식품 등 광고 플랫폼 수익" },
    { k: "결제 수수료", body: "쇼핑 · 옵션 결제에서 발생하는 PG 마진" },
    { k: "DB 인사이트", body: "개인정보 제거 후 집계 · 추후 · 보안 서버 전제" },
  ],
  exclude: [
    { k: "물류 상품 매출", why: "상품 판매액 자체는 공급사 매출 — 앱 매출이 아니다. 결제 수수료만 잡는다" },
    { k: "대행 · 상조 · 매칭 서비스 매출", why: "업체 지급 · 인건비로 나가는 몫" },
    { k: "환불 · 취소 거래", why: "발생하지 않은 매출" },
    { k: "투자금 · 대출금", why: "매출이 아니다" },
    { k: "환자 연결 수수료", why: "의료법 위반 — 아예 하지 않는다 (매출로 잡지도 않는다)" },
  ],
  base: "모든 매출은 부가가치세를 제외한 공급가 기준으로 산정한다.",
};

export const PROFIT_SPLIT = [
  { k: "매출", v: "100%", tone: "info", note: "위 4개 항목 합계 (공급가 기준)" },
  { k: "− 비용", v: "인건비 · 서버 · AI · 임차 · 솔루션 · 보안 · 영업/마케팅", tone: "bad", note: "" },
  { k: "− 세금", v: "", tone: "bad", note: "" },
  { k: "= 수익", v: "", tone: "ok", note: "" },
  { k: "− 유보금", v: "10%", tone: "warn", note: "회사 재투자 몫 — 넥스트를 위한 재원" },
  { k: "참여자 배분", v: "각 22.5%", tone: "ok", note: "남은 90%를 4인이 25%씩" },
];

export const GOVERNANCE = [
  { k: "지급 방식", body: "월정 · 익월 20일 정산 지급. 배당이 아니라 기타소득(3.3%) 또는 급여로 처리" },
  { k: "의사결정", body: "비용 항목 신설 · 매출 정의 변경 등은 4인 중 3인 동의" },
  { k: "앱 소유권", body: "회사(쿨롱) 귀속. 참여자는 지분이 아니라 수익 배분 권리를 갖는다" },
  { k: "역할", body: "총괄 / 개발 / 공동개발 · 유치 / 중개 · 영업조직 유치" },
  { k: "미확정", body: "계약 형태(공동사업계약 vs 사용권·소유권 분리) · 유통 경로 — 다음 미팅" },
];
...
// 옵션 — 기본은 가볍게, 수익은 옵션에서. 이것이 확정 구조의 전제다.
export const OPTIONS_CATALOG = [
  { k: "추가 병원 동행 (건당)", price: "11만", attach: 38, note: "연 1회 포함분 초과 — 가장 높은 부착" },
  { k: "추가 안심방문 (건당)", price: "6.5만", attach: 31, note: "월 1회 포함분 초과 · 여름 · 겨울 수요 급증" },
  { k: "보호자 역할 대행", price: "건당 9만", attach: 14, note: "원거리 자녀 대신 병원 보호자 역할 · 새벽 초동" },
  { k: "구매대행 (장보기 · 생활용품)", price: "수수료 8%", attach: 44, note: "저항이 가장 적은 옵션 — 박카스 · 생활용품 (일반의약품 제외)" },
  { k: "대리운전 연계", price: "중개 수수료", attach: 6, note: "제휴 준비 중 · 긴급 초동 이동" },
];

// 확정 구조의 손익 — 정직하게 본다
export const PRICING_UNIT = {
  head: "기본 구독만으로는 역마진 — 옵션 부착이 손익분기의 조건",
  rows: [
    { k: "월 구독료 (1인)", v: "+55,000", tone: "ok", note: "고객 결제액 · 2026-09-11 확정" },
    { k: "대리점망 수당 (60%)", v: "−33,000", tone: "bad", note: "직판 전환 시 −19,250" },
    { k: "회사 실입금", v: "22,000", tone: "info", note: "여기서 원가를 뺀다" },
    { k: "안심방문 월 1회 원가", v: "−65,000", tone: "bad", note: "회의 실측 원가" },
...
export const PRICING_DECISIONS = [
  ...
  { k: "유통 경로 선택", why: "대리점망 60% vs 직판 35% — 회사 실입금이 2배 차이", opt: "① 대리점망(속도) ② 직판(마진) ③ 병행", state: "미확정 — 다음 미팅", tone: "bad" },
...
export const EXEC_PRIORITY = [
  ...
  { u: "now", k: "유통 경로 확정 — 대리점망 vs 직판", why: "수당 60% vs 35% · 회사 실입금이 2배 차이. 가격은 확정, 이것만 남았다",
...
// ════ 컨시어지 — 방문 전 확인전화 (2026-08-21 시트 컨시어지 오늘 3번) ════
//
...
// 컨시어지 → 관제 메시지 (2026-08-21 시트 컨시어지 오늘 2번).
// 어르신의 즉시방문요청과 같은 자리·같은 성격 — 현장에서 관제를 부르는 버튼이다.
export const OPS_MESSAGE_PRESETS = [
  "도착이 늦어질 것 같습니다",
  "어르신 상태가 평소와 다릅니다",
  "주차·출입에 문제가 있습니다",
  "일정 변경 요청을 받았습니다",
