...
export const GLASS_DEVICES = [
  { id: "A", k: "엔터프라이즈 글래스", ex: "RealWear · Vuzix 급", price: "250만", state: "대안",
    pros: ["안드로이드 — 우리 앱을 기기에 올린다", "MDM 원격 관리 · 핫스왑 배터리", "음성 제어 (부축 중에도 손이 자유롭다)", "관제 실시간 원격 지원"],
    cons: ["산업용 외형 — 케어 현장에 과하다", "무게 · 초기 투자 10,000가구 기준 13.7억"], tone: "info" },
  { id: "B", k: "중급 카메라 글래스", ex: "일상 안경형 · 앱 연동", price: "80만", state: "권장",
    pros: ["일상 안경 외형 — 어르신 앞에서 위압적이지 않다", "우리 앱(폰)과 페어링 — 데이터 경로를 우리가 통제", "50g대 · 2시간 동행 커버"],
    cons: ["디스플레이 없음 (AR 오버레이 불가)", "배터리 2시간 — 예비 필수"], tone: "ok" },
  { id: "C", k: "컨슈머 글래스", ex: "Ray-Ban Meta 류", price: "35만", state: "불가",
    pros: ["가장 트렌디 · 외형 거부감 최소", "가격이 싸다"],
    cons: ["영상이 제조사 클라우드를 경유 — 우리가 통제 못 한다", "파이프라인 API 없음 · 클립 길이 제한", "민감정보 처리 위탁 요건을 못 맞춘다"], tone: "bad" },
];
// 규모별 — 권장안(B · 공용 배치 · 구간 녹화 8분 · 720p H.265 · 30일 보관) 기준
export const GLASS_SCALE = [
  { n: "1,000가구", staff: "189명", units: "55대", capex: "4,400만", opex: "313만", per: "3,130원",
    detail: [["기기 감가 (3년)", "122만"], ["분실 · 파손 (연 8%)", "29만"], ["배터리 · 소모품", "4만"], ["클라우드 · 서버 · 보안", "58만"], ["기기 운영 인력", "100만"]] },
  { n: "5,000가구", staff: "943명", units: "273대", capex: "2.18억", opex: "1,062만", per: "2,124원",
    detail: [["기기 감가 (3년)", "606만"], ["분실 · 파손 (연 8%)", "145만"], ["배터리 · 소모품", "18만"], ["클라우드 · 서버 · 보안", "93만"], ["기기 운영 인력", "200만"]] },
  { n: "10,000가구", staff: "1,887명", units: "546대", capex: "4.37억", opex: "1,986만", per: "1,986원",
    detail: [["기기 감가 (3년)", "1,214만"], ["분실 · 파손 (연 8%)", "291만"], ["배터리 · 소모품", "36만"], ["클라우드 · 서버 · 보안", "145만"], ["기기 운영 인력", "300만"]] },
];
// 녹화 방식이 비용을 결정한다 — 화질 × 길이 × 보관기간은 곱해진다
...
export const GLASS_FIELD = [
  ...
  { k: "한마디로 중단", body: "어르신이 “지금 끄세요” 하면 음성 명령으로 즉시 중단 · 그 사실도 기록에 남긴다", tone: "ok" },
...
export const GLASS_VALUE = [
  { k: "관제 실시간 원격 지원", body: "수습 컨시어지의 시야를 시니어 코치가 원격으로 보며 지시한다. 마포지점 수습 비중 50% 문제를 사람 파견 없이 푼다", ir: "채용 → 투입 리드타임 31일 단축의 지렛대", tone: "ok" },
  { k: "1인칭 시점 기록", body: "의사가 무엇을 가리켰는지 · 어르신이 무엇을 보고 불안해했는지가 남는다. 가슴 바디캠은 시선이 어긋난다", ir: "리포트 신뢰도 · 분쟁 방어", tone: "ok" },
  { k: "핸즈프리", body: "부축 · 휠체어 조작 중에도 음성으로 시작 · 중단. 손이 묶이는 순간이 정확히 기록이 필요한 순간이다", ir: "안전 사고 0건 유지의 증빙", tone: "ok" },
  { k: "케어 데이터 자산", body: "1인칭 영상은 AI 진화 E4(처방) 단계의 조치–결과 학습에 쓸 수 있는 유일한 원본", ir: "데이터 해자 — 후발 주자가 못 따라오는 축적", tone: "ok" },
];
export const GLASS_PHASES = [
  { d: "1단계 · 4주", k: "기기 선정 · 실물 검증", body: "후보 3종 대여 — 2시간 연속 녹화 배터리 · 무게 · 어르신 반응 · 앱 연동 가능 여부", who: "개발 · 운영" },
  { d: "2단계 · 6주", k: "병원 MOU 촬영 조항", body: "제휴 6곳 재협상에 촬영 조항 추가. 이게 없으면 기기를 사도 진료실에서 못 쓴다", who: "제휴" },
  { d: "3단계 · 4주", k: "동의 · 파기 체계", body: "민감정보 별도 동의 · 30일 자동 파기 · 열람 승인제 · 감사 로그. PIA에 항목 포함", who: "법무 · 개발" },
  { d: "4단계 · 8주", k: "1개 지점 파일럿", body: "강남 본점 20대. 거부율 · 중단 요청 빈도 · 실제 활용률 · 분실 파손률 측정", who: "운영" },
  { d: "5단계", k: "확대 판단", body: "거부율 20% 초과 또는 활용률 40% 미만이면 바디캠으로 회귀. 기준을 먼저 정해 두고 시작한다", who: "경영" },
];
