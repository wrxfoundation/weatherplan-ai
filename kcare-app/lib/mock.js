...
export const OUTING = {
  ...
  adviceGuardian:
    "병원 근처 미세먼지가 나쁨입니다. 어머니께 KF94 마스크와 양산을 안내했고, 컨시어지 준비물에도 반영했습니다.",
};
...
// "건강 → FIT 건강기록, 과거기록 보이게"). 마지막 두 값은 TODAY_ME(오늘 3,140 ·
// 어제 2,880 걸음)와 같은 수를 쓴다 — 화면 두 곳이 다른 숫자를 말하면 안 된다.
export const FIT_WEEK = [
  { ago: 6, steps: 4120, sleep: 7.1 },
  { ago: 5, steps: 3660, sleep: 6.8 },
  { ago: 4, steps: 2210, sleep: 5.9 },
  { ago: 3, steps: 3980, sleep: 7.0 },
  { ago: 2, steps: 3450, sleep: 6.4 },
  { ago: 1, steps: 2880, sleep: 6.6 },
  { ago: 0, steps: 3140, sleep: 6.2 },
];
...
// 선생님 메시지함 — 어르신 ↔ 담당 컨시어지 음성 메시지 (2026-08-28 요청).
//
// 보관 원칙: 보낸 지 24시간이 지나면 목록에서 저절로 사라진다. 어르신 댁의
// 목소리가 서버에 쌓이지 않게 하는 것이 기본값이고, 남기고 싶은 것만 어르신이
// 직접 내려받아 본인 폰에 둔다. 내려받기는 '받은 메시지'에만 붙는다 —
// 보낸 것은 어르신이 이미 원본을 말씀하신 것이라 되돌려 받을 것이 없다.
//
...
// 내려받기용 데모 음성 파일 (public/demo/). 데모 빌드라 실제 녹음이 아니라
// 짧은 확인음이고, 파일명에 '데모'를 남겨 실제 통화 녹음과 헷갈리지 않게 한다.
export const TEACHER_DEMO_FILE = "/demo/voice-demo.wav";


// 오늘의 세대공감 — 매일 한 가지씩 바뀌는 이야깃거리 (2026-08-28 화이트보드).
// 목소리 메시지를 보내실 때 "무슨 말을 하지"가 가장 큰 장벽이라, 오늘 할 말을
// 하나 드린다. 퀴즈·음악·뉴스·옛날 이야기가 돌아가고, 자녀에게 그대로 물어보실
// 수 있게 한 문장(prompt)을 같이 준다.
//
// 날짜로 고르므로(아래 rollingTopics) 같은 날 여러 번 열어도 같은 순서다 —
// 열 때마다 바뀌면 "아까 그거 뭐였지"가 된다.
export const GEN_TOPICS = [
  {
    kind: "퀴즈",
    title: "요즘 아이들이 쓰는 말",
    body: "손주가 '갓생 산다'고 하면 무슨 뜻일까요? 부지런하게 잘 산다는 뜻입니다.",
    prompt: "손주에게 '갓생이 무슨 뜻이냐' 물어보시면 신나서 설명해 줍니다.",
  },
  {
    kind: "음악",
    title: "그 시절 그 노래",
    body: "1974년 이 노래가 라디오를 뒤덮었습니다 — 김추자 '님은 먼 곳에'.",
    prompt: "이 노래 아느냐고 자녀에게 물어보세요. 모르면 들려주시면 됩니다.",
  },
  {
    kind: "뉴스",
    title: "오늘 이런 일이",
    body: "요즘은 은행에 가지 않고 휴대폰으로 통장을 만듭니다.",
    prompt: "자녀에게 '너는 은행 언제 마지막으로 갔니' 물어보세요.",
  },
  {
    kind: "옛날 이야기",
    title: "그때는 그랬지",
    body: "예전엔 전화 한 통 하려고 우체국 앞에 줄을 섰습니다.",
    prompt: "그 시절 이야기를 목소리로 남겨 주세요. 손주가 좋아합니다.",
  },
  {
    kind: "퀴즈",
    title: "몸에 좋은 상식",
    body: "물은 한 번에 많이 드시는 것보다 조금씩 자주 드시는 게 좋습니다.",
    prompt: "자녀에게 '물 잘 챙겨 마시냐' 한마디 남겨 보세요.",
  },
  {
    kind: "손주 말",
    title: "ㅋㅋ 와 ㅇㅇ",
    body: "손주가 'ㅋㅋ'라고 보내면 웃는다는 뜻이고, 'ㅇㅇ'는 '응, 알았어'라는 뜻입니다.",
    prompt: "손주에게 'ㅋㅋ' 하고 한 번 보내 보세요. 깜짝 놀랍니다.",
  },
  {
    kind: "요즘 것",
    title: "짧은 영상",
    body: "요즘 아이들은 15초짜리 짧은 영상을 넘겨 가며 봅니다. 재미없으면 바로 다음으로 넘깁니다.",
    prompt: "손주에게 '요즘 뭐가 제일 재밌냐' 물어보세요.",
  },
  {
    kind: "요즘 것",
    title: "인생네컷",
    body: "요즘 아이들은 사진관 대신 길거리 사진 부스에서 넉 장짜리 사진을 찍습니다.",
    prompt: "손주 만나면 같이 한 장 찍자고 해 보세요. 아주 좋아합니다.",
  },
  {
    kind: "손주 말",
    title: "찐친 · 인싸",
    body: "'찐친'은 진짜 친한 친구, '인싸'는 사람들과 잘 어울리는 사람이라는 뜻입니다.",
    prompt: "손주에게 '네 찐친은 누구냐' 물어보세요. 이야기가 길어집니다.",
  },
  {
    kind: "옛날 이야기",
    title: "그때 그 간식",
    body: "쫀드기와 아폴로를 아십니까. 손주는 이름도 못 들어 봤을 것입니다.",
    prompt: "어릴 적 제일 좋아하던 군것질을 목소리로 남겨 주세요.",
  },
  {
    kind: "요즘 것",
    title: "식당 주문 기계",
    body: "요즘 식당은 화면을 눌러 주문합니다. 어려우면 직원을 부르셔도 됩니다.",
    prompt: "손주에게 '그 기계 쓰는 법 좀 알려 달라' 해 보세요.",
  },
  {
    kind: "옛날 이야기",
    title: "삐삐를 아십니까",
    body: "번호만 남기면 공중전화를 찾아 뛰어가던 시절이 있었습니다.",
    prompt: "그때 무슨 번호를 남겼는지 손주에게 이야기해 주세요.",
  },
  {
    kind: "요즘 것",
    title: "손주의 장래희망",
    body: "요즘 아이들은 영상 만드는 사람, 게임 하는 사람도 직업으로 삼습니다.",
    prompt: "손주에게 '너는 커서 뭐가 되고 싶냐' 물어보세요.",
  },
  {
    kind: "음악",
    title: "요즘 노래",
    body: "손주가 듣는 노래는 대개 3분이 안 됩니다. 짧고 빠릅니다.",
    prompt: "손주에게 '요즘 제일 좋아하는 노래 하나 들려 달라' 해 보세요.",
  },
];

// 오늘부터 n개 — 가족 탭에서 옆으로 밀어 보는 롤링용 (2026-08-28 요청
// "하루에 1개 말고 롤링 형태로 5개"). 첫 장이 그날의 이야기라
// 날짜가 바뀌면 목록 전체가 한 칸씩 돌아간다.
export function rollingTopics(n = 5, d = new Date()) {
  const dayNum = Math.floor(
    Date.UTC(d.getFullYear(), d.getMonth(), d.getDate()) / 86400000
  );
  const len = GEN_TOPICS.length;
  return Array.from({ length: Math.min(n, len) }, (_, i) => GEN_TOPICS[(dayNum + i) % len]);
}

// 자녀 확인 기록 — 상대 시간만, 미확인 카운트 금지 (06 §3.10). GUARDIANS 페르소나와 동일 인물.
// 2026-08-28 시트로 어르신 가족 탭의 "자녀들이 보고 있습니다" 카드는 삭제했다 —
// 지켜보고 있다는 말이 안심이 아니라 감시로 읽힐 수 있다. 보호자 화면에서는 그대로 쓴다.
...
// 기본 보증 범위 밖의 옵션 (회의 9 · REQ-04 야간 외주 · REQ-14 보험)
//
// 여기에는 "아직 팔 수 없는 것"만 둔다. 병원 동행 · 추가 방문처럼 단가가 확정된
// 상품은 해주세요 카탈로그(lib/requests.js)가 유일한 출처다 — 같은 상품의 값을
// 두 곳에 두면 한쪽만 고쳐지고, 화면에 서로 다른 금액이 뜬다.
//
// 요금은 지어내지 않는다. 야간 출동은 파트너 계약이, 보험은 GA 등록이 선행이라
// 둘 다 확정된 값이 없다 (미결 11 · 12번).
export const OPTION_SERVICES = [
  {
    key: "night",
    name: "야간 출동 (외주)",
    desc: "야간 긴급 출동 — 외주 파트너 수행 · 가구별 가입 옵션",
    price: "요금 확정 전",
    locked: false,
    lockNote: "외주 파트너 선정·책임 범위 계약 후 요금 확정 (미결 11)",
    // 가입 가구에서만 관제 조치 옵션으로 나타난다
    gatesDispatch: true,
  },
  {
    key: "insurance",
    ...
    price: "준비 중",
    locked: true,
    lockNote: "GA 등록 후 오픈 — 등록 전 모집 금지 (H4 · 기능 플래그 잠금)",
  },
];
...
export const CARE_TEAM = {
  ...
  trust:
    "두 분 다 신원조회와 배상책임보험을 마쳤습니다. 방문 전날 저녁에 사진과 이름을 보내 드리니 문 앞에서 확인하세요.",
};
...
// 컴포넌트(보호자·어르신 공용 안부 음성)로 대체되면서 쓰이지 않게 됐다.
