...
//
// 원칙
//  · 관제가 엑셀로 검산할 수 있어야 한다. 그래서 공식을 1:1 로 옮겼고, 시트의 특이점
//    (최대연령 미사용 · 소득은 감점만 · 의료급여 미사용 — 아래 주석 ※)도 그대로다.
//    한 군데(주거형태 미확인 처리)만 다른 항목과 같은 규칙으로 맞췄다 — scorePolicy 주석.
//    시트 테스트 고객 79건 판정·점수·순서가 같은 것을 확인했다 (세션 검증 · 커밋 메시지).
...
export function scorePolicy(p, c) {
  ...
  const t = {
    ...
    // 주거형태 — 시트 공식과 한 군데 다르다 (2026-09-09 · 사용자 위임 판단).
    // 시트는 주거형태가 '미확인'이면 무주택·주택보유 요건이 없는 정책까지 0.5 로 깎아서
    // 79건이 통째로 '추가확인'이 되고, 요건이 있는데 미확인이면 0.5 가 아니라 0(불일치)이
    // 된다. 다른 열네 항목은 전부 "요건이 있을 때만 본다"라서 이 열만 형식이 어긋난다 —
    // 공식 실수로 보고 다른 항목과 같은 규칙으로 맞췄다: 요건 없음 1 · 요건 있음 → 맞으면 1 ·
    // 미확인 0.5 · 아니면 0. 시트 테스트 고객은 '무주택'이라 이 차이를 타지 않는다 —
    // 79건 검산은 그대로 일치한다. 실무진이 시트대로를 원하면 이 블록만 되돌리면 된다.
    housing: e.noHome
      ? c.housing === "무주택" ? 1 : c.housing === "미확인" ? 0.5 : 0
      : e.ownHome
      ? c.housing === "자가" ? 1 : c.housing === "미확인" ? 0.5 : 0
      : 1,
