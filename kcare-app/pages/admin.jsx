...
function StatTile({ k, v, color = NAVY, note }) {
  return (
    <div className="rounded-xl border border-navy/[.06] bg-white/60 px-3.5 py-3">
      ...
      <div className="mt-0.5 font-num text-[20px] font-bold" style={{ color }}>
        {v}
      </div>
      ...
      <div className="console-bg min-h-screen px-4 pb-10 pt-7 text-ink sm:px-8">
        <div className="mx-auto max-w-[1240px] space-y-4">
          ...
          <header>
            ...
            <h1 className="mt-0.5 text-[26px] font-bold tracking-[-.01em] text-navy">사람 · 경영 총괄</h1>
            <p className="mt-1.5 max-w-[84ch] text-[13px] leading-[1.75] text-muted">
          ...
          <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))" }}>
            {PEOPLE_KPIS.map((k) => (
              <Panel key={k.k} className="!p-4">
                ...
                <div className="mt-1 font-num text-[22px] font-bold" style={{ color: k.color }}>
                  {k.v}
                </div>
                <div className="mt-0.5 text-[10px] text-muted">{k.sub}</div>
              </Panel>
            ))}
          </div>
          ...
          {tab === "staff" && (
            <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))" }}>
              <Panel className="min-w-0">
                <PanelHead title="케어 품질 분포" right={<span className="text-[11px] text-muted">평점 출처: 가족 만족도</span>} />
              ...
              <Panel className="min-w-0">
                <PanelHead title="HR 워치 — 이직 · 번아웃 신호" right={<span className="text-[11px] text-muted">사람 관리 · 사건 아님</span>} />
                  ...
                  {STAFF_HR_WATCH.map((r) => (
                    <div key={r.name} className="rounded-xl border border-navy/[.06] bg-white/60 px-3.5 py-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[12px] font-bold text-navy">{r.name}</span>
                      </div>
              ...
              <Panel className="min-w-0">
                <PanelHead title="케어 품질 인센티브 구성" right={<span className="text-[11px] text-muted">업셀 인센티브 대체 (원칙 1)</span>} />
                <div className="mt-3 space-y-3">
                  {INCENTIVE_MIX.map((m) => (
                ...
                <div className="mt-4 border-t border-navy/[.08] pt-3">
                  <div className="text-[11px] font-bold text-navy">자격 · 교육 현황</div>
          ...
          {tab === "family" && (
            <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))" }}>
              <Panel className="min-w-0">
                <PanelHead title="가입 퍼널 (최근 90일)" right={<span className="text-[11px] text-muted">주보호자 가입 → 초대 링크 → 부보호자</span>} />
                <div className="mt-3 space-y-3">
                  {FAMILY_FUNNEL.map((f) => (
              ...
              <Panel className="min-w-0">
                <PanelHead title="보호자 참여도" right={<span className="text-[11px] text-muted">참여도가 유지율의 선행 지표</span>} />
              ...
              <Panel className="min-w-0">
                <PanelHead title="이탈 위험 세그먼트" right={<span className="text-[11px] text-muted">개별 가구 연락은 CS 소관</span>} />
                <div className="mt-3 space-y-2.5">
                  {CHURN_SEGMENTS.map((c) => (
                    <div key={c.seg} className="rounded-xl border border-navy/[.06] bg-white/60 px-3.5 py-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[12px] font-bold text-navy">{c.seg}</span>
                        <span className="ml-auto font-num text-[12px] font-bold text-ink">{c.n}</span>
                      </div>
                      <div className="mt-1 text-[11px] font-bold leading-[1.55] text-ink">조치 — {c.action}</div>
                    </div>
                  ))}
                </div>
              </Panel>
          ...
          {tab === "care" && (
            <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))" }}>
              <Panel className="min-w-0">
                <PanelHead title="케어 성과 지표 (월)" right={<span className="text-[11px] text-muted">서비스의 결과 — 사람 지표의 합</span>} />
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {CARE_OUTCOMES.map((c) => (
              ...
              <Panel className="min-w-0">
                <PanelHead title="월간 안전 집계" right={<span className="text-[11px] text-muted">개별 사건 비노출 · 집계만</span>} />
                <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <Panel className="min-w-0">
                <PanelHead title="어르신 구성" right={<span className="text-[11px] text-muted">전체 132명</span>} />
                <div className="mt-3 space-y-3">
                  {ELDER_MIX.map((m) => (
                ...
                <div className="mt-4 flex gap-2 border-t border-navy/[.08] pt-3">
                  {ELDER_RISK_MIX.map((r) => (
                    <div key={r.k} className="flex-1 rounded-xl border border-navy/[.06] bg-white/60 px-3 py-2.5 text-center">
                      <div className="font-num text-[18px] font-bold" style={{ color: r.color }}>
                        {r.n}
                      </div>
                      <div className="mt-0.5 text-[10px] font-bold text-muted">{r.k}</div>
                    </div>
                  ))}
                </div>
                <p className="mt-3 border-t border-navy/[.08] pt-2.5 text-[10px] leading-[1.7] text-muted">
