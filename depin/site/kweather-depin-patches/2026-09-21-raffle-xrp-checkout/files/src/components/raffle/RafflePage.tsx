    ...
    <div className="wb-page full-bleed">
          ...
          <div className="panel" style={{ display: "flex", flexWrap: "wrap", gap: 18, alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              <div style={{ fontSize: 14.5, color: "var(--ink-dim)" }}>
                ...
                {!done && mine && mine.status !== "OVERFLOW" && (phase === "OPEN"
            ...
            {!done && phase === "OPEN" && mine?.status !== "OVERFLOW" && (
      ...
      <section className="sec-pad" style={{ paddingTop: 0 }}><div className="wrap">
        ...
        {[
          ...
          t({ ko: ["래플 NFT 는 무엇인가요?", "응모 자격을 증명하는 XRPL NFT(XLS-20)이며, 행사장에서 경품을 수령할 때 사용됩니다. 계정당 1장 발급되며 추첨은 결제가 확정된 응모 전원을 대상으로 진행됩니다. 행사 초대권은 당첨자에게 별도 안내됩니다."], en: ["What is the raffle NFT?", "An XRPL NFT (XLS-20) that proves your entry and is used to collect your prize at the venue. One is issued per account, and the draw covers every confirmed entry. Event invitations are sent separately to winners."], ja: ["ラッフルNFTとは？", "応募資格を証明するXRPL NFT（XLS-20）で、会場での賞品受取に使用します。1アカウントにつき1枚発行され、抽選は決済が確定した応募全員を対象に行います。招待券は当選者に別途ご案内します。"], zh: ["什么是抽奖 NFT？", "证明参与资格的 XRPL NFT（XLS-20），用于在会场领取奖品。每账户发放一张，抽奖面向所有已确认支付的参与者。活动邀请函将另行通知中奖者。"], es: ["¿Qué es el NFT del sorteo?", "Un NFT de XRPL (XLS-20) que acredita su participación y se usa para recoger el premio en el recinto. Se emite uno por cuenta y el sorteo incluye a todas las participaciones confirmadas. Las invitaciones se comunican por separado a los ganadores."] }),
