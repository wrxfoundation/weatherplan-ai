...
/* 1g 배송 대기 (데스크톱) + 1l (모바일 세로 타임라인) — PRD §3, §6.3 (8/26 개정:
   구매 확인 2요소 = 결제 지갑 주소 + 난수 주문번호. 배송 접수는 발송 2주 전부터 공지되는
   별도 폼에서 지갑 주소·주문번호·성함·연락처·배송지만 접수, 배송 후 파기. KO/EN 토글 지원) */
      ...
      <main className="wl-sec-pad" style={{ display: "flex", flexDirection: "column", gap: 32, alignItems: "center", background: "linear-gradient(var(--panel), #fff)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, alignItems: "center", textAlign: "center" }}>
          ...
          <h2 className="big-title" style={{ fontWeight: 800, color: "var(--w-deep)" }}>
            {en
              ? <>Genesis Number <span style={{ color: "var(--w-main)" }}>#{order.genesisNo}</span> has been assigned</>
        ...
        {/* 구매 확인 2요소(지갑 주소 + 주문번호) · 접수 방식 · 환불 안내 */}
        <div className="w720" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 14, border: "1.5px solid var(--w-main)", background: "var(--w-tint)", borderRadius: 12, padding: "15px 18px", flexWrap: "wrap" }}>
            <div style={{ display: "flex", gap: 26, flexWrap: "wrap" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                <span style={{ fontSize: 11.5, fontWeight: 800, letterSpacing: ".04em", color: "var(--w-main)" }}>{en ? "PAYING WALLET" : "결제 지갑 주소"}</span>
                <span className="mono" style={{ fontSize: 16.5, fontWeight: 800, color: "var(--w-deep)" }}>{order.wallet}</span>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                <span style={{ fontSize: 11.5, fontWeight: 800, letterSpacing: ".04em", color: "var(--w-main)" }}>{en ? "ORDER NUMBER" : "주문번호"}</span>
                <span className="mono" style={{ fontSize: 16.5, fontWeight: 800, color: "var(--w-deep)" }}>{order.id}</span>
              </div>
            </div>
            <span style={{ fontSize: 12, lineHeight: 1.6, color: "var(--ink-4)", maxWidth: 300 }}>
              {en
                ? <>These two are your <b style={{ color: "var(--w-deep)" }}>proof of purchase</b>. The order number is shown only here and when you reconnect your wallet — keep it safe.</>
                : <>이 두 가지가 <b style={{ color: "var(--w-deep)" }}>구매자 확인 수단</b>입니다. 주문번호는 이 화면과 지갑 재연결로만 확인됩니다 — 꼭 보관하세요.</>}
            </span>
          </div>
          <div style={{ display: "flex", gap: 12, border: "1px solid var(--bd-card)", borderRadius: 12, padding: "14px 18px", fontSize: 13, lineHeight: 1.7, color: "var(--ink-4)", alignItems: "flex-start" }}>
            ...
            <span>
              {en ? (
                <>
                  <b style={{ color: "var(--ink-2)" }}>How shipping intake works.</b> Starting 2 weeks before devices ship, we announce the intake form on our official Telegram and X → enter your <b style={{ color: "var(--ink-2)" }}>wallet address · order number · name · phone · shipping address</b>, and units ship in order. We collect only what delivery requires and <b style={{ color: "var(--ink-2)" }}>delete it after delivery</b>. Your redeem code for product registration is on the card inside the box.
                </>
              ) : (
                <>
                  <b style={{ color: "var(--ink-2)" }}>배송 접수는 이렇게 진행됩니다.</b> 디바이스 발송 2주 전부터 공식 텔레그램·X로 접수 폼을 알려드립니다 → 폼에 <b style={{ color: "var(--ink-2)" }}>지갑 주소 · 주문번호 · 성함 · 연락처 · 배송지</b>를 입력하면 순서대로 발송합니다. 배송에 필요한 정보만 받고, <b style={{ color: "var(--ink-2)" }}>배송이 끝나면 파기</b>합니다. 정품 등록용 리딤코드는 박스 안 카드에 있습니다.
                </>
              )}
            </span>
          </div>
