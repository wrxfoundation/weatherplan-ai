/* 인맥 지도 (9/11 서우 — "인맥지도 vercel 배포 아직 안 했는데 추가해줘")
   ...
   ── 발송 여부는 이 파일이 관리하지 않는다 (9/17 신설) ──────────────────
   보냈는지 아닌지는 **서우만 안다**. Claude 는 저장소만 보므로 「발송 대기」를 상태로 들고 있으면
   **이미 처리된 것을 대기로 착각해 같은 재촉을 반복**한다(9/17 실제로 발생). 그래서 여기 적는 것은
   **「무엇을 하기로 했는가」까지**다. `next` 는 결정이지 할 일 목록이 아니고, tie 의 「발송」 표기는
   서우가 알려줄 때만 갱신한다. 답장이 오면 그때 stance 를 올린다.
...
export const PEOPLE: Person[] = [
  ...
  },
  {
    ...
    lane: "impact", stance: "talking", tie: "1촌 · 인사 왕복 · 답장 발송(9/17)",
  ...
  {
    id: "jennyk", name: "Jenny Kim", org: "Binance", role: "Content Manager(2026-03~) · Binance Square 콘텐츠 파트너십·3개국어(CN/EN/KR) 호스트 — APAC 커뮤니티·마케팅·오프라인 이벤트 (싱가포르)",
    lane: "exchange", stance: "open", tie: "1촌 성사(9/21) · 알림 DM 초안 완료(`business-directions.md` Jenny 절) — 발송은 서우 통보 시 갱신", group: "Binance", via: "공통 1촌 Lauren·Ken", meet: "KBW 9/29~10/1 · XRP SEOUL 10/3 부스",
    ...
    next: "**★ 판정 수정(9/17 서우) — 지금 보낸다, 「알림」까지만.** 내가 「10/3 뒤에」로 걸었으나 **타이밍이 거꾸로였다**: **행사는 알릴 명분이지 알린 뒤의 결과물이 아니다.** 끝난 뒤의 「우리 거기 나왔어요」는 뉴스가 아니고, **전에 알려야 그가 그 행사를 인지해 끝난 뒤 「어땠어요」가 이어진다 — 그게 2depth 고 그 문은 지금만 열린다.** 플래티넘 스폰서는 **지금 이미 사실**이라 기다릴 이유가 없다. **무게는 Alexander 보다 가볍게** — 회사 2문장, 구조 설명(11소스·median·원장) 전부 제외. **콘텐츠 협업을 우리가 제안하지 않는다**(그러면 유료 영업 프레임에 걸린다) — 관심 있으면 **그가 먼저 묻고 그게 훨씬 강한 자리**다. 바이낸스 네 번째 문이지만 **Alexander 는 감시·규제, 이쪽은 콘텐츠·이벤트로 하는 말이 완전히 다르고 무게도 다르다.** **9/21 수락 → DM 확정**: 회사 2문장 + 「이번 주에 인사하는 이유」(플래티넘 스폰서 · 9/18 키노트 공식 · 기기 무대) + 부스 초대 한 줄 + 긍정형 맺음. 콘텐츠 협업·토큰·체인 이름·구조 설명 0. 영어로 보내고 한국어 답이 오면 한국어로.",
  },
  {
