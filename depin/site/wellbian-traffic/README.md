...
건드리지 않는다. 텔레봇의 `/traffic` 은 9/8 판 그대로 남아 있다 — **9/26 서우 「열어두쇼」: 텔레봇 `/traffic` 도, 이 사이트도 열어 둔다**
(텔레봇 `TRAFFIC_PUBLIC` · 여기 `TRAFFIC_KEY` 둘 다 넣지 않음). 채널 판정 9/26 수정은 여기에만 있어서, 두 화면의 KOL · 기타 리퍼럴 숫자는 다르게 나온다.
...
| `/sources` 소스별 일자 | **세션 소스 × 날짜 표.** 줄 = 세션 소스(GA 화면과 같은 이름), 칸 = 그날 그 소스로 들어온 **사용자**(기본, 9/26 서우 「사용자수 기준이야」) 또는 세션. 일별/주별 · 소스 찾기 · 채널 칩 · 날짜 머리를 누르면 그날 많은 순 · 칸을 누르면 그날 전체에서의 몫. 소스·합계 열 고정, 처음엔 최근 날짜로 넘겨 둔다, 오늘 열은 점선(처리 지연). 원자료 내려받기 버튼. |
| `/export?t=…&f=…` | 내려받기(아래). |
...
- **사용자(9/26)** — 사용자는 더할 수 없다(사흘 온 한 사람을 날짜별로 더하면 3명). 그래서 칸(날짜 × 소스)은 GA 값 그대로, 합계 열(소스의 기간
  사용자) · 합계 줄(날짜별 전체 사용자) · 총계는 GA 에 따로 물어 **중복을 뺀 값**을 쓴다 — 칸을 더한 값보다 작은 것이 정상이다. 주별도 GA 의
  ISO 주(월요일 시작, `isoYearIsoWeek`) 값을 따로 받는다. 보고서 6개(`activeUsers` — 날짜×소스 · 소스 · 날짜 · 전체 · ISO 주×소스 · ISO 주)를
  세션 원자료와 나란히 부르고 따로 실패한다 — 못 읽으면 화면은 세션만, 주별만 못 읽으면 사용자 주별만 막힌다. 화면에서 찾기·채널로 걸러도
  사용자 합계 줄은 전체(중복 제거) 그대로다 — 거른 소스끼리의 중복 제거 합은 GA 에 다시 묻지 않으면 알 수 없다.
...
매체 `community`(9/22 부터 커뮤니티 채널 링크 — yunlog · HANNA · GOMAE 등)도 KOL(9/26 실데이터에서 확인).

## 내려받기 — `/export?t=…&f=csv|csv16|xlsx`

`t` = `daily` · `weekly` · `monthly`(채널을 열로) · `channels` · `sources` · `srcusers`(소스×일자 사용자, 9/26) · `srcdaily`(소스×일자 세션) ·
`raw`(원자료) · `content` · `campaigns` · `pages` · `all`(구역으로 나눈 한 파일). `f=xlsx` 는 표 전부를 시트 12장으로(**기본 추천** — 어디서 열어도 한글이 안 깨진다,
「소스×일자(사용자)」가 「소스×일자(세션)」 앞),
