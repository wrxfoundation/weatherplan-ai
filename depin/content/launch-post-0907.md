...
**커미션 방침(9/10 결정)**: 이 링크들은 **유입 측정용**이지 판매 귀속용이 아니다 — 결제 완료 매칭·환불 회수·지급 처리가 없다. 1차 판매(9/15~16)에는 KOL 커미션도 고객 할인코드도 없다(쿠폰 기능 부재 + 예약자 역차별). **커미션은 2차 판매에서 리퍼럴 추적을 붙인 뒤 프로그램으로 연다** — 근거·부수 조건은 `depin/intel/business-directions.md` 「KOL 커미션·할인 방침 결정(9/10)」.
