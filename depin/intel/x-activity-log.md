...
⚠️ **핸들 불일치 경보**: 8/23 로스터 확인 때 "실핸들 = @SotaOnchain, @WatanabeSota 아님"으로
기입했는데 이번 유입이 @WatanabeSota 명의 — 둘 중 하나가 오류(당시 오판 or 사칭 계정). 서우
프로필 실사 요청(팔로워·인증·개설일). 사칭이면 패러디 규칙상 더욱 무반응. 텍스처 1건만(핸들
확정 전 귀속 보류): "미국이 토큰화 선도, 최대 수요는 전통 접근이 막힌 아시아" = 일본 클러스터
2단계 배경 논지(의견, 팩트 아님).
