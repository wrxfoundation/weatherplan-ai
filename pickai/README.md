...
URL 하나로 결정론 체크 40개 + AI 크롤러 13종 접근성 + 정밀 진단(LLM 심사, 베타 무료)을 받고,
...
| `lib/compare.js` | 사이트 나란히 비교 — 격차 추출·순위·명령서 섹션 (순수 함수) |
...
진단 결과 화면에서 비교할 주소를 최대 2곳까지 넣으면 **같은 40개 항목**으로 함께 진단해 비교한다.
경쟁사 판정 로직은 없다 — 무엇을 비교할지는 전적으로 사용자가 정한다(경쟁사, 업계 1등, 계열사, 우리 사이트의 개편 전후 등).

확산 영역의 소셜 채널 감지에는 레딧·쿠오라·미디엄이 포함되며, AI 인용 상위 소스에 연결돼 있는지를 별도로 표시한다.

- 총점 순위, 영역별 점수·격차, **상대는 통과했는데 우리는 실패/주의인 항목** 목록을 보여준다
- 격차 목록은 심각도 → 앞선 사이트 수 → 가중치 순으로 정렬되어, 위에서부터 처리하면 된다
- 비교 결과는 **AI 명령서(.md)에도 자동으로 포함**된다
- 서버 함수를 새로 만들지 않고 기존 `/api/scorecard`를 사이트 수만큼 병렬 호출한다
  (호출마다 자기 타임아웃을 갖기 때문에 한 함수에 몰아넣는 것보다 안전하다)
- 일부 사이트만 실패해도 성공한 곳끼리 비교하고, 실패는 해당 입력칸에 사유를 표시한다
- 상대 사이트도 **공개된 첫 화면 1개만** 본다 — 로그인 영역·하위 페이지는 진단하지 않는다
- 비교 대상의 **페이지 성격이 다르면**(홈 vs 글) 경고를 띄운다 — 점수 차이가 성격 차이일 수 있기 때문

비교 로직은 네트워크를 타지 않는 순수 함수(`lib/compare.js`)라 단위 테스트가 가능하다.
...
- **푸터 영상은 단색화되어 재생된다.** 원본이 어떤 색이든(황금색 등) 명암만 남기고 브랜드 톤으로 물들여
  네이비 배경과 충돌하지 않게 한다. `styles/globals.css`에서 조절:
  - `--footer-video-hue` — 색조 (기본 `140deg` 네이비 계열 · `125deg` 청록 쪽 · `175deg` 파랑 쪽)
  - `--footer-video-sat` — 채도 (기본 `1.45`, 낮추면 더 무채색)
  - `--footer-video-filter: none` — 단색화를 끄고 원본 색 그대로 재생
