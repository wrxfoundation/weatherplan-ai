...
| `lib/compare.js` | 경쟁사 나란히 비교 — 격차 추출·순위·명령서 섹션 (순수 함수) |
...
진단 결과 화면에서 경쟁사 주소를 최대 2곳까지 넣으면 **같은 39개 항목**으로 함께 진단해 비교한다.

- 총점 순위, 영역별 점수·격차, **경쟁사는 통과했는데 우리는 실패/주의인 항목** 목록을 보여준다
- 격차 목록은 심각도 → 앞선 경쟁사 수 → 가중치 순으로 정렬되어, 위에서부터 처리하면 된다
...
- 경쟁사도 **공개된 첫 화면 1개만** 본다 — 로그인 영역·하위 페이지는 진단하지 않는다
