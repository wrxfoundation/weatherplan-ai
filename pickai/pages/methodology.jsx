      ...
      <div className="max-w-[980px] mx-auto px-6" style={{ paddingTop: 40, paddingBottom: 80 }}>
        ...
        <Card style={{ borderLeft: `3px solid ${T.coralDark}` }}>
          <ul style={{ color: T.charcoal, fontSize: 13.5, lineHeight: 1.9, paddingLeft: 18, margin: 0 }}>
            ...
            <li><strong>로드맵</strong> — ① 실제 AI 인용률·브랜드 언급률(SOV) 반복 프로빙 ② Core Web Vitals 랩 측정 ③ 다중 페이지 감사(사이트맵 기반). <em>구현 완료: 진단 이력·점수 추이(DB), 자체 벤치마크 축적·실측 백분위, 주간 자동 재진단·변동 알림, AI 명령서 프레임워크 맞춤 지침, 경쟁사 나란히 비교.</em></li>
            <li><strong>경쟁사 비교의 경계</strong> — 경쟁사도 공개된 첫 화면 1개만 같은 39개 항목으로 재며, 로그인이 필요한 영역이나 하위 페이지는 보지 않는다. 따라서 &ldquo;사이트 전체의 우열&rdquo;이 아니라 &ldquo;대표 페이지의 AI 검색 대비 상태&rdquo; 비교로 읽어야 한다. 두 사이트를 같은 시점에 같은 기준으로 재기 때문에 점수 자체는 그대로 견줄 수 있다.</li>
          </ul>
        </Card>
