...
test("멱등성: 같은 체인 머리는 두 번 앵커하지 않는다", async () => {
  ...
  // 첫 시도: TSA 응답이 쓰레기 → 저장 거부(예외) → 앵커 없음
  await assert.rejects(() => anchorSource({ sourceId: "bunyang-capsule", dataDir, fetchImpl: fakeFetch, log: () => {} }), /TimeStampResp/);
  ...
  assert.equal(calls, 1, "이미 앵커된 머리에는 네트워크 호출이 없어야 한다");
});
