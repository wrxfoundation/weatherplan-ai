...
function computeOuting({ date = "", location, origin, mobility } = {}) {
  // origin만 온 경우: 유령 '도착지 서울'을 만들지 않고 단일 지점으로 (사용자가 말한 지점만)
  if (origin && !location) return computeLegOuting({ date, location: origin, mobility });
...
export function executeCareTool(name, input = {}) {
  switch (name) {
    ...
    case "create_booking": {
      ...
      // 예약 식별자에 날짜·시간·병원·매니저 전부 반영 — 동일 조건 재예약도 유일 ID (결정론 유지)
      const seed = hashStr(`${mgr.id}|${input.date}|${input.time}|${input.hospital}|${q.hours}`) % 9000 + 1000;
...
export function mergeCareProfile(prior = {}, patch = {}) {
  ...
  if (b) {
    ...
    const isSameBooking = p.last_booking?.id === b.id; // 같은 예약 재병합 시 방문수 불변 (멱등)
    p.last_booking = { id: b.id, date: b.date, time: b.time, hours: b.hours };
    if (!isSameBooking) p.visits = (p.visits || 0) + 1;
  }
