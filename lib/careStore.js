...
export async function fetchTodayBookings() {
  ...
  const r = await pg("bookings", {
    query: `?select=id,service_type,manager_key,manager_name,hospital,date_text,hours,price,status,outing&created_at=gte.${today.toISOString()}&order=created_at.desc&limit=30`,
  });
