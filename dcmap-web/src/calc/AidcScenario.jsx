...
export default function AidcScenario({ mw, nonCapital }) {
  ...
  let verdict
  if (!nonCapital) {
    verdict = { tone: 'bad', head: '수도권 — 면제 비대상', body: '수도권 입지는 특별법 면제 대상이 아니며 전력계통영향평가에서 억제(감점) 적용. 비수도권 전환 시 면제 트랙이 열립니다.' }
  } else if (!psiaTarget) {
    verdict = { tone: 'good', head: '10MW 미만 — 평가 비대상', body: '전력계통영향평가 대상(10MW 이상) 자체가 아닙니다. 면제 논의 이전 단계.' }
  } else if (exemptEligible) {
    verdict = { tone: 'good', head: '면제 가능 (가정 상한 이내)', body: `비수도권 + 계약전력 ${round}MW ≤ 가정 상한 ${cap}MW → 2027.3~ 전력계통영향평가 면제 트랙. 심의 리드타임·불확실성이 크게 줄어드는 구간.` }
  } else {
    verdict = { tone: 'warn', head: '상한 초과 — 면제 제외(가정)', body: `계약전력 ${round}MW > 가정 상한 ${cap}MW → 비수도권 가점은 받되 면제에서는 제외되는 시나리오. 상한이 ${round}MW 이상으로 정해져야 면제 대상.` }
  }
    ...
    <div className="calc-card aidc-card">
      <div className="econ-head" style={{ marginTop: 0 }}>
        <span className="chart-title" style={{ margin: 0 }}>
          AIDC 특별법 면제 시나리오 — {round}MW · {nonCapital ? '비수도권' : '수도권'}
        </span>
        <span className="econ-flag">대통령령 미정 · 가정 시나리오</span>
      </div>
      <p className="chart-note" style={{ marginTop: 2 }}>
        비수도권 AIDC는 2027.3~ <Term k="전력계통영향평가">전력계통영향평가</Term> 면제 대상이나, 면제 <b>용량 상한은 대통령령(미제정)</b>에 위임돼 있습니다.
        아래 상한을 조정해 이 용량·입지가 면제되는지, 비수도권 후보가 얼마나 열리는지 시나리오로 확인하세요.
      </p>

      <div className="aidc-cap-row">
        <label className="lbl-cap" htmlFor="aidc-cap">가정 면제 상한 <b className="aidc-cap-val">{cap}MW</b></label>
      ...
      <div className="spec-grid" style={{ marginTop: 10 }}>
        <div className="spec-cell">
          ...
          <div className="v">{psiaTarget ? '대상 (10MW+)' : '비대상 (10MW 미만)'}</div>
        </div>
        <div className="spec-cell">
          <div className="k">이 시나리오 면제 여부</div>
          <div className="v">{exemptEligible ? '면제 가능 (2027.3~)' : overCap ? '상한 초과 — 제외' : '비수도권 아님 — 제외'}</div>
        </div>
        <div className="spec-cell">
          <div className="k">이 용량 수용 비수도권 후보</div>
          <div className="v">
            {nonCapCandidates.toLocaleString()}곳
            <span className="muted" style={{ marginLeft: 6, fontSize: '0.85em' }}>계통 여유 ≥ {round}MW · 산단 기준</span>
          </div>
        </div>
        <div className="spec-cell">
          <div className="k">면제 시 기대 효과</div>
          <div className="v">{exemptEligible ? '심의 리드타임·불확실성↓' : '해당 없음(현 시나리오)'}</div>
        </div>
      </div>
