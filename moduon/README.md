...
| 카테고리 상세 (S-02) | `/category/internet` 등 | 상품·지원금 비교 — 인터넷/TV는 통신사별 요금표·설치 절차·사은품 정책·FAQ 포함 |
...
| AI 생활비 진단 (C-05) | `/diagnosis` | 1분 문답(건너뛰기=미평가) → 절감 추정·알뜰 백분위 → 절감 실행서(.md) 다운로드 |
...
- **탐라몰(인터넷/TV)**: 통신사별 요금 매트릭스 · 신청→사은품 4단계 절차 · 사은품 지급 정책 · FAQ → `/category/internet` (`src/lib/telco.js`)
- **선행 서비스 4종(kcare·axion·pickai·dcmap)**: 리드 전이 테이블 가드 · ?ref= 추천 귀속 · 어드민 파생 알림센터 · 견적 공유 링크(URL 프리필) · 진단 미평가/실행서 다운로드 · AI 프록시 검증/레이트리밋 등 — 상세 교훈과 실서비스 전환 체크리스트는 **`docs/PRODUCTION_NOTES.md`**
