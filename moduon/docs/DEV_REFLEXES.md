...
- **`--replace` 는 원본이 무엇인지 확인한 뒤에만 쓴다** — 파일명이 "차량목록"이라 현대 페이지인 줄 알고
  5개 저장본에 `--replace` 를 걸었는데, 그 중 현대는 하나도 없어서(기아 2 · 제네시스 · KGM · 중고차)
  기존 현대 20건이 통째로 날아갔다. 저장본은 `Snapshot-Content-Location` 헤더에 원래 URL 이 남으니
  **먼저 그것부터 읽는다.** 커밋 직후라 `git checkout -- ` 로 복구했다. 기본값이 병합인 이유가 이것이다.
- **부분일치 매칭은 짧은 이름이 긴 이름의 것을 훔쳐간다** — "스포티지"가 문서에서 먼저 나온
  "스포티지 하이브리드" 카드를 집어가 엉뚱한 사진이 붙었다. 이름표 매칭은 **정확 일치를 먼저** 돌리고,
  남은 것만 부분일치로 줍되 "다른 차종과 정확히 일치하는 이름표"는 건너뛴다. 부분일치로 주운 건은
  따로 출력해 눈으로 보게 한다. 목록 페이지처럼 파생 모델(하이브리드·EV·특장)이 널린 곳에서는 반드시 필요하다.
- **크롬의 "웹페이지, 단일 파일(.mhtml)" 은 그냥 읽으면 안 된다** — quoted-printable MIME 묶음이라
  `<img src=3D"…">` 로 깨져 있고 이미지가 base64 로 같이 들어 있어 파일이 5~8MB 다. text/html 파트만
  골라 `=XX`·소프트 줄바꿈을 되돌리고 latin1→utf8 로 재조립해야 평범한 HTML 이 된다. 사용자가 Ctrl+S 로
  주는 파일의 기본 형식이므로 파서 쪽에서 흡수하는 게 맞다.
- **alt 가 비면 "주변 텍스트"가 아니라 카드 구조를 읽는다** — 목록 카드는 `<img>` 바로 뒤가 모델명이고
  그 다음이 값(리스 월 N 만원 / 렌트불가)이다. 값 표기에서 자르면 앞부분이 정확히 모델명이다.
  앞뒤 600자에서 한글 덩어리를 긁는 추정은 위 스포티지 사고의 원인이었다.
