...
export default function Home({ tenant }) {
  ...
  return (
    <main>
      ...
      {/* ── CTA 밴드 (파스텔 그린 · 좌상단 진한 엣지 + 유리 두께감 + 대리석 마블링) ── */}
      <section className="marble relative mt-6 overflow-hidden rounded-section" style={{ background: 'linear-gradient(135deg,#DFF3E8 0%,#C2E7D2 100%)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.6), inset 0 -1px 1px rgba(23,120,70,0.10), 0 12px 34px rgba(23,178,106,0.20)' }}>
...
function BenefitCard({ style, edge, light, label, amountNum, obj, onClick }) {
  return (
    <button
      onClick={onClick}
      style={style}
      className="marble glass-sheen group relative flex min-h-[150px] flex-col overflow-hidden rounded-card p-5 text-left transition-transform duration-200 hover:-translate-y-[3px] sm:min-h-[296px] sm:p-6"
    >
