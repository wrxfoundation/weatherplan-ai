...
// ─── 프레스룸: 보도자료 자동 생성 (언론 자동화) ──────────────────────
// kind: open(신규 분양몰) · record(월간 실적) · biz(외국인 파일럿) · brand(플랫폼 소개)
// 실데이터(stats)만 소재로 쓰고 확인 불가한 주장·최상급 표현은 만들지 않는다.
// 유료 게재형 기사의 '광고' 표기 등 매체 준수는 배포 채널 단계의 몫.
export function pressRelease(a) {
  const facts = `활성 분양몰 ${a.active}곳 · 총판 ${a.distributors}개 권역 계약 · 이번 달 개통 완료 ${a.done}건 · 누적 상담 리드 ${a.leads}건 · 파트너 월 매출 합계 ${a.salesText}`
  const ANGLE = {
    open: `신규 지역 파트너(분양몰) 개소 소식. 새 파트너: ${a.mallName}(${a.mallRegion}). 지역 밀착 개통·정착지원 프로그램(권역 예산 20%)과 연결해 서술`,
    record: `${a.monthLabel} 운영 실적 공개. 사은품 '지급 명단 공개' 정책(투명성)과 개통 완료 건수를 중심으로 서술`,
    biz: `외국인 개통 지원 파일럿 착수. 다국어 AI 상담·체류기간 연동 약정 설계·내외국인 동일 가격 원칙을 중심으로 서술(목표 수치는 "계획"으로 명시)`,
    brand: `생활서비스 비교·개통 플랫폼 소개. 인터넷·휴대폰·정수기·보험 통합 비교, AI 상담(모비), 지역 분양몰 네트워크 구조를 중심으로 서술`,
  }
  const prompt = `당신은 기업 홍보팀의 보도자료 작성자입니다. 아래 [확인된 사실]만 사용해 한국어 보도자료 초안을 작성하세요.\n\n[확인된 사실]\n${facts}\n\n[이번 자료의 앵글]\n${ANGLE[a.kind]}\n\n[형식]\n1) 제목(사실 기반 1줄) 2) 부제 3) 리드문("생활서비스 비교·개통 플랫폼 '모두온'은 …라고 ${a.dateLabel} 밝혔다") 4) 본문 2~3문단(역피라미드, 수치 근거) 5) 관계자 인용 1개 6) '■ 모두온 소개' 3줄 7) '■ 문의' 1줄(press@moduon.example)\n\n[금지] 최상급·과장(최초/1위/최저가 등), 확인 불가한 수치 창작, 경쟁사 비방. 전체 700~900자.`
  return { prompt, task: 'press', local: localPress(a) }
}
function localPress(a) {
  const H = {
    open: {
      t: `모두온, ${a.mallRegion} 신규 분양몰 '${a.mallName}' 오픈`,
      s: `— 활성 분양몰 ${a.active}곳으로 확대 · 권역 예산 20% 정착지원 프로그램 연계 —`,
      lead: `${a.mallRegion} 권역에 신규 지역 파트너 '${a.mallName}'를 열었다고 ${a.dateLabel} 밝혔다.`,
      b1: `이번 오픈으로 모두온의 활성 분양몰은 ${a.active}곳, 총판 계약 권역은 ${a.distributors}개가 됐다. 신규 파트너는 인터넷·휴대폰·정수기 등 생활서비스 비교 견적과 개통 상담을 지역 기반으로 제공한다.`,
      b2: `모두온은 신규 파트너의 초기 정착을 위해 권역 매출의 20%를 재원으로 하는 정착지원 프로그램(로컬 마케팅 매칭·교육)을 함께 운영한다.`,
      q: `"개통은 결국 동네 신뢰가 만드는 거래입니다. 지역 파트너가 빨리 자리 잡도록 본사가 예산으로 함께 뜁니다."`,
    },
    record: {
      t: `모두온, ${a.monthLabel} 개통 완료 ${a.done}건 — 사은품 지급 명단 공개 지속`,
      s: `— 누적 상담 리드 ${a.leads}건 · 파트너 월 매출 합계 ${a.salesText} —`,
      lead: `${a.monthLabel} 개통 완료 ${a.done}건을 기록하고 사은품 지급 명단 공개 정책을 이어간다고 ${a.dateLabel} 밝혔다.`,
      b1: `모두온은 개통 고객에게 약속한 사은품의 지급 내역을 명단(마스킹)으로 공개하고 있다. 조건 충족 시 지급되는 금액과 시점을 사전에 고지하고, 실제 지급 기록으로 증빙하는 방식이다.`,
      b2: `현재 활성 분양몰 ${a.active}곳과 총판 ${a.distributors}개 권역이 상담·개통·정산을 분담하며, 누적 상담 리드는 ${a.leads}건이다.`,
      q: `"'돌려받는 돈'은 광고 문구가 아니라 지급 기록으로 말해야 한다고 믿습니다."`,
    },
    biz: {
      t: `모두온, 외국인 개통 지원 파일럿 착수`,
      s: `— 다국어 AI 상담 · 체류기간 연동 약정 설계 · 내외국인 동일 가격 원칙 —`,
      lead: `국내 체류 외국인을 위한 개통 지원 파일럿을 시작한다고 ${a.dateLabel} 밝혔다.`,
      b1: `파일럿은 다국어 AI 상담(모비), 체류기간에 맞춘 약정 설계와 위약금 시뮬레이션, 서류 체크리스트 안내를 제공한다. 요금·혜택 표기는 내외국인 동일 원칙을 적용한다.`,
      b2: `모두온은 외국인 밀집 권역의 분양몰 ${a.active}곳 네트워크를 활용해 대면 상담을 병행할 계획이다. 90일 파일럿의 목표와 확대 여부는 실측 결과로 결정한다는 계획이다.`,
      q: `"언어·서류·약정 장벽을 상품으로 바꾸는 것이 이번 파일럿의 목표입니다."`,
    },
    brand: {
      t: `생활서비스 비교·개통 플랫폼 '모두온', 분양몰 네트워크로 확장`,
      s: `— 인터넷·휴대폰·정수기·보험 통합 비교 · 활성 분양몰 ${a.active}곳 —`,
      lead: `인터넷·휴대폰·정수기·보험 등 생활서비스를 한 번에 비교·개통하는 플랫폼 운영 현황을 ${a.dateLabel} 소개했다.`,
      b1: `모두온은 AI 상담(모비)으로 견적·진단을 자동화하고, 지역 파트너 '분양몰' ${a.active}곳이 상담과 개통을 맡는 구조다. 이번 달 개통 완료는 ${a.done}건, 누적 상담 리드는 ${a.leads}건이다.`,
      b2: `사은품 지급 명단 공개, 조건 사전 고지 등 투명성 정책을 운영하며, 총판 ${a.distributors}개 권역과 정산을 분담한다.`,
      q: `"비교부터 개통·정산까지 한 줄로 잇는 것이 모두온의 방식입니다."`,
    },
  }[a.kind]
  return [
    `[보도자료] ${H.t}`, H.s, '',
    `생활서비스 비교·개통 플랫폼 '모두온'은 ${H.lead}`, '',
    H.b1, '', H.b2, '',
    `모두온 관계자는 ${H.q}라고 말했다.`, '',
    `■ 모두온 소개 — 인터넷·휴대폰·정수기·보험 등 생활서비스를 한 번에 비교하고, 지역 파트너 '분양몰' 네트워크로 상담·개통·정산까지 연결하는 플랫폼. 사은품 지급 명단 공개로 '돌려받는 돈'의 투명성을 지향한다.`,
    `■ 문의 — 모두온 홍보팀 press@moduon.example`,
  ].join('\n')
}
