...
;(async () => {
  ...
  // 사이드 배너 = 브랜드 > 기종 체크박스 트리 (아정당 구조)
  const tree = await page.locator('[data-t="shop-tree"]').innerText()
  check(tree.includes('삼성') && tree.includes('애플'), '트리: 브랜드 2')
  for (const m of ['Z 폴드8', 'Z 플립8', 'S26 울트라', '아이폰 17 프로']) check(tree.includes(m), `트리 기종: ${m}`)
  check(await page.locator('[data-t="shop-tree"] [data-t="tree-item"]').count() === 10, '트리 항목 = 브랜드 2 + 기종 8')

  const item = (t) => page.locator('[data-t="shop-tree"] [data-t="tree-item"]', { hasText: t }).first()
  await item('삼성').click(); await page.waitForTimeout(150)
  check(await cards() === 5, '삼성 체크 → 5')
  check(await item('Z 폴드8').getAttribute('aria-pressed') === 'true', '브랜드 체크가 하위 기종까지 켬')
  await item('Z 폴드8').click(); await page.waitForTimeout(150)
  check(await cards() === 4, '하위 1종 해제 → 4')
  await page.locator('[data-t="shop-filters"] button', { hasText: '초기화' }).click(); await page.waitForTimeout(150)
  check(await cards() === 8, '초기화 → 8')
  await item('애플').click(); await page.waitForTimeout(150)
  await page.locator('[data-t="shop-filters"] button', { hasText: /^1TB$/ }).click(); await page.waitForTimeout(150)
  check(await cards() === 2, '애플 + 1TB → 2 (프로·프로맥스)')
  await page.locator('[data-t="shop-filters"] button', { hasText: '초기화' }).click()
  await page.locator('[data-t="shop-filters"] button', { hasText: /^전체$/ }).click(); await page.waitForTimeout(150)

  await page.locator('[data-t="shop-cur"] button', { hasText: /^KT$/ }).click(); await page.waitForTimeout(250)
  check(page.url().includes('cur=KT'), '현 통신사 선택이 URL 에 실림')

  // 카드에 3사 전부 — 저렴한 순, 첫 줄에 최저 배지
  const first = page.locator('[data-t="shop-card"]').first()
  check(await first.locator('[data-t="card-offer"]').count() === 3, '카드에 3사 전부')
  const rows = await first.locator('[data-t="card-offer"]').allInnerTexts()
  for (const c of ['SKT', 'KT', 'LG U+']) check(rows.some((r) => r.includes(c)), `3사 표기: ${c}`)
  check(rows[0].includes('최저'), '첫 줄에 최저 배지')
  check(rows.some((r) => r.includes('기기변경')), 'KT 사용 중 → KT 줄은 기기변경')
  check(rows.filter((r) => r.includes('번호이동')).length === 2, '나머지 2사는 번호이동')
  const money = rows.map((r) => (r.match(/([\d,]+)원/g) || []).map((x) => Number(x.replace(/[^\d]/g, ''))))
  check(money.every((m) => m.length === 2 && m[0] < m[1]), `줄마다 실구매가 < 정가 (${money.map((m) => m.join('<')).join(' / ')})`)
  check(rows.every((r) => /\d+%/.test(r)), '줄마다 할인율')
  // 통신사 줄을 누르면 그 조건으로 상세가 열린다
  await first.locator('[data-t="card-offer"]').nth(1).click(); await page.waitForTimeout(700)
  check(/carrier=/.test(page.url()) && /join=/.test(page.url()), `줄 클릭 → 상세에 조건 전달 (${decodeURIComponent(page.url().split('?')[1] || '')})`)
  const sel = await page.locator('[data-t="detail-carriers"] button[aria-pressed="true"]').innerText()
  check(sel.length > 0, `상세가 그 통신사로 열림 (${sel.replace(/\n/g, ' ')})`)
