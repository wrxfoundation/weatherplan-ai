...
;(async () => {
  ...
  check(await page.locator('[data-t="car-card"]').count() === 24, `현대 차종 ${await page.locator('[data-t="car-card"]').count()}종 (승용 20 + 상용 4)`)
  ...
  check(!rentOnly.includes('렌트상담') && !rentOnly.includes('렌트불가'), '렌트 필터 → 금액 없는 차종 제외')
  ...
  const gridKinds = await page.evaluate(() => [...document.querySelectorAll('[data-t="car-grid"] [data-t="car-photo"]')].map((e) => ({ kind: e.dataset.kind, src: e.dataset.src })))
  check(gridKinds.length === 24, `목록 이미지 슬롯 ${gridKinds.length}개`)
  check(gridKinds.every((g) => g.kind === 'list'), '목록은 썸네일 변형(_list)')
  check(gridKinds.filter((g) => g.src).every((g) => /^\/assets\/cars\/[a-z0-9-]+\.jpg$/.test(g.src)), '목록 경로 형식 = /assets/cars/{id}.jpg')
  check(gridKinds.some((g) => !g.src), '매핑 없는 차종은 경로 없이 SVG 실루엣')
  ...
  const gal = await page.evaluate(() => { const e = document.querySelector('[data-t="car-gallery"]'); return e && Number(e.dataset.shots) })
  check(gal === 3, `갤러리 슬롯 ${gal}장`)
  check(!(await page.content()).includes('acrentcar.com'), '상세에도 제휴사 도메인 핫링크 없음')
